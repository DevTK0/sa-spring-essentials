...
public class AccountClientTests {
        ...
        public void listAccounts_using_valid_user_should_succeed() {
                ...
                ResponseEntity<Account[]> responseEntity = restTemplate.withBasicAuth("user", "user")
        ...
        public void listAccounts_using_valid_admin_should_succeed() {
                ...
                // we have to use Account[] instead of List<Account>, or Jackson won't know what
                // type to unmarshal to
                ResponseEntity<Account[]> responseEntity = restTemplate.withBasicAuth("admin", "admin")
                                .getForEntity(url, Account[].class);
                assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
                Account[] accounts = responseEntity.getBody();
                assertThat(accounts.length >= 21).withFailMessage("Expected 21 accounts, but found " + accounts.length)
        ...
        public void createAccount_using_admin_should_return_201() {
                ...
                ResponseEntity<Void> responseEntity = restTemplate.withBasicAuth("admin", "admin")
        ...
        public void createAccount_using_user_should_return_403() throws Exception {
                ...
                Account account = new Account(number, "John Doe");
                account.addBeneficiary("Jane Doe");
                ResponseEntity<Void> response = restTemplate.withBasicAuth("user", "user")
                                .postForEntity(url, account, void.class);
        ...
        public void addAndDeleteBeneficiary_using_superadmin_should_succeed() {
                ...
                URI newBeneficiaryLocation = restTemplate.withBasicAuth("superadmin", "superadmin")
